...
func Signup() gin.HandlerFunc {
	return func(c *gin.Context) {
		...
		if err != nil {
			c.IndentedJSON(http.StatusBadRequest, gin.H{"error": "username is already taken!"})
			return
		}
	...
	return func(c *gin.Context) {
		...
		if err := row.Scan(&foundUser.ID, &foundUser.Name, &foundUser.Password, &foundUser.Token, &foundUser.Refresh_token, &foundUser.User_type); err != nil {
			if err == sql.ErrNoRows {
				c.IndentedJSON(http.StatusNotFound, gin.H{"message": "password or username is incorrect"})
				return
			}
